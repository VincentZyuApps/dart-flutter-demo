#include "include/desktop_integration_vincentzyu/desktop_integration_vincentzyu_plugin.h"
...
#define DESKTOP_INTEGRATION_VINCENTZYU_PLUGIN(obj)                       \
  (G_TYPE_CHECK_INSTANCE_CAST((obj),                                     \
                              desktop_integration_vincentzyu_plugin_get_type(), \
                              DesktopIntegrationVincentzyuPlugin))

struct _DesktopIntegrationVincentzyuPlugin {
...
G_DEFINE_TYPE(DesktopIntegrationVincentzyuPlugin,
              desktop_integration_vincentzyu_plugin,
              g_object_get_type())
...
static GtkWindow* toplevel_window(DesktopIntegrationVincentzyuPlugin* self) {
...
static void activate_window(DesktopIntegrationVincentzyuPlugin* self,
...
static void handle_method_call(FlMethodChannel* channel,
                               ...
                               gpointer user_data) {
  (void)channel;
  DesktopIntegrationVincentzyuPlugin* self =
      DESKTOP_INTEGRATION_VINCENTZYU_PLUGIN(user_data);
...
static void desktop_integration_vincentzyu_plugin_dispose(GObject* object) {
  DesktopIntegrationVincentzyuPlugin* self =
      DESKTOP_INTEGRATION_VINCENTZYU_PLUGIN(object);
  g_clear_object(&self->channel);
  G_OBJECT_CLASS(desktop_integration_vincentzyu_plugin_parent_class)
      ->dispose(object);
}

static void desktop_integration_vincentzyu_plugin_class_init(
    DesktopIntegrationVincentzyuPluginClass* klass) {
  G_OBJECT_CLASS(klass)->dispose = desktop_integration_vincentzyu_plugin_dispose;
}

static void desktop_integration_vincentzyu_plugin_init(
    DesktopIntegrationVincentzyuPlugin* self) {
  (void)self;
}

void desktop_integration_vincentzyu_plugin_register_with_registrar(
    FlPluginRegistrar* registrar) {
  DesktopIntegrationVincentzyuPlugin* plugin =
      DESKTOP_INTEGRATION_VINCENTZYU_PLUGIN(g_object_new(
          desktop_integration_vincentzyu_plugin_get_type(), nullptr));
  plugin->view = fl_plugin_registrar_get_view(registrar);
  g_autoptr(FlStandardMethodCodec) codec = fl_standard_method_codec_new();
  plugin->channel = fl_method_channel_new(
      fl_plugin_registrar_get_messenger(registrar),
      "desktop_integration_vincentzyu/methods", FL_METHOD_CODEC(codec));
